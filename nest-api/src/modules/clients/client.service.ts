...
export class ClientService {
  ...
  ): Promise<[{ client: ClientModel; purchaseCount: number }[], number]> {
    //Possibility to add pagination later
    // Get all clients and its associated purchase counts

    const [clients, totalCount] =
      await this.clientRepository.getAllClients(input);
    //Map through each client to get their purchase count
    const clientsWithPurchaseCounts = await Promise.all(
      clients.map(async (client) => {
        const [, purchaseCount] = await this.salesRepository.getClientPurchases(
          client.id,
        );
  ...
  public async GetClientInfo(
    id: string,
  ): Promise<[ClientModel, BookModel[], number] | undefined> {
    const client = await this.clientRepository.findClient(id);
    if (!client) {
      return undefined;
    }

    const [booksId, purchaseCount] =
      await this.salesRepository.getClientPurchases(id);
    if (booksId.length === 0) {
      return [client, [], purchaseCount];
    }

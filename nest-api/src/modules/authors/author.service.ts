...
export class AuthorService {
  ...
  ): Promise<[{ Authors: AuthorModel; Number_of_Books: number }[], number]> {
    const [authors, totalCount] =
      await this.authorRepository.getAllAuthors(input);
    const AuthorsWithBookNumber = await Promise.all(
      authors.map(async (author) => {
        const [, booksCount] = await this.bookRepository.GetAuthorBooks(
          author.id,
        );
  ...
  public async getAuthorInfo(
    id: string,
  ): Promise<[AuthorModel, BookModel[], number, number] | undefined> {
